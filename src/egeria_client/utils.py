...
class MessageDefinition():
    def __init__(self, message_id: str, message_template: str, system_action: str, user_action: str, params: [str]):
    ...
    def __str__(self):
        return ("messageId=" + self.message_id +
                ...
                ", systemAction=" + self.system_action +
...
class ExceptionMessageDefinition(MessageDefinition):
    def __init__(self, http_error_code: str, message_id: str, message_template: str,
                 system_action: str, user_action: str, params: [str]):
        MessageDefinition.__init__(self, message_id, message_template,
                                   system_action, user_action, params)
        self.http_error_code = http_error_code

    def __str__(self, ):
        return ("http_error_code=" + self.http_error_code +
...
class InvalidParameterException(Exception):
    ...
    def __init__(self, message, class_name, action_description, param_name):
        if param_name is not None:
            self.message = message.message_template % param_name
        else:
...
def validate_user_id(user_id: str, method_name):
    if (user_id is None) or len(user_id) == 0:
        msg = ExceptionMessageDefinition(OMAGCommonErrorCode.SERVER_URL_NOT_SPECIFIED.value['http_error_code'],
...
def validate_url(url: str, class_name, method_name) -> bool:
    if (url is None) or (len(url) == 0):
        ...
        raise InvalidParameterException(msg, class_name, method_name, None)

    result = validators.url(url)
    # print(f"validation result is {result}")
    if result is not True:
        msg = ExceptionMessageDefinition(OMAGCommonErrorCode.SERVER_URL_MALFORMED.value['http_error_code'],
        ...
        raise InvalidParameterException(msg, class_name, method_name, "URL")
    else:
        return True
...
def get_asset_universe(serverName, serverPlatformName, serverPlatformURL, serviceURLMarker, userId, assetGUID):
    connectedAssetURL = serverPlatformURL + '/servers/' + serverName + '/open-metadata/common-services/' \
                        + serviceURLMarker + '/connected-asset/users/' + userId
...
def get_comments(serverName, serverPlatformName, serverPlatformURL, serviceURLMarker, userId, assetGUID):
    connectedAssetURL = serverPlatformURL + '/servers/' + serverName + '/open-metadata/common-services/' + serviceURLMarker + '/connected-asset/users/' + userId
...
def get_comment_replies(serverName, serverPlatformName, serverPlatformURL, serviceURLMarker, userId, assetGUID,
                        commentGUID):
    connectedAssetURL = serverPlatformURL + '/servers/' + serverName + '/open-metadata/common-services/' + serviceURLMarker + '/connected-asset/users/' + userId
...
def get_api_operations(serverName, serverPlatformName, serverPlatformURL, serviceURLMarker, userId, apiSchemaTypeGUID):
    connectedAssetURL = serverPlatformURL + '/servers/' + serverName + '/open-metadata/common-services/' + serviceURLMarker + '/connected-asset/users/' + userId
...
def get_schema_attributes_from_schema_type(serverName, serverPlatformName, serverPlatformURL, serviceURLMarker, userId,
                                           schemaTypeGUID):
